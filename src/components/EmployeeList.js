...
const EmployeeList = () => {
   ...
   const deleteEmployee = (e, id) =>{
    ...
    EmployeeService.deleteEmployee(id).then( (res)=>{
        // console.log("deleteEmployee => "+employees);
        if(employees){
  ...
  return (
    ...
    <div className='container mx-auto my-8'>
      <div className='h-12'>
        <button onClick={()=> navigate("/addEmployee")} className='rounded bg-slate-700 text-white px-2 py-2 font-semibold'>Add Employee</button>
      </div>
      <div className='flex shadow border-b'>
        <table className='min-w-full'>
            ...
            {/* loading should be completed then show this */}
            {!loading &&(
            <tbody className='bg-white'>
                {employees.map((employee) => (
                    <Employee employee={employee} deleteEmployee = {deleteEmployee} viewEmployee = {viewEmployee} key={employee.id}/>
                ))}
            </tbody>
            ) }
        </table>
